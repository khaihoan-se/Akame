import { FORMATS } from '@/constants/en';
...
const FormatSelector: React.FC<SelectorProps> = ({
    title,
    RightIcon,
}) => {
    ...
    const handleGetItem = (event: any) => {
        ...
        router.query.format = event.currentTarget.textContent
        router.push(router)
        setOpen(false)
    }
    const handRemove = () => {
    ...
    return (
        <div className="space-y-2 relative">
            ...
            {open && <div className="no-scroll w-full absolute bg-background-800 z-40 px-4 py-2 max-h-[300px] overflow-hidden overflow-y-auto rounded-md">
                {
                    FORMATS.map((item: any, index: number) => (
